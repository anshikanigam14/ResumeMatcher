...
def get_ranked_results(k, top_k_resumes, rephrased_query):
    model = load_model()
    top_k_ranked_resume = rank_resumes(model, rephrased_query, top_k_resumes, k)
    # print(top_ranked[0].keys())
    
    return top_k_ranked_resume
...
# top_ranked = get_ranked_results(k, top_k_resumes, rephrased_query)
# print(top_ranked[0])